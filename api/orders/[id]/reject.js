...
module.exports = withGerente(async (req, res) => {
  ...
  try {
    ...
    if (order.estado !== 'PENDIENTE') return res.status(400).json({ error: 'Solo se pueden rechazar órdenes en estado PENDIENTE' });

    await query(
      `UPDATE ordenes SET estado='RECHAZADA', gerente_id=$1, gerente_nombre=$2, firma_gerente=$3, notas=$4 WHERE id=$5`,
      [req.user.id, req.user.nombre, firma_gerente.trim(), notas.trim(), id]
    );
