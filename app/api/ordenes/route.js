...
export async function GET(request) {
  ...
  const rows = await query(`
    SELECT o.id, o.hotel, o.proveedor, o.estado, o.firma_encargado, o.firma_gerente,
           o.notas_gerente, o.nro_orden, o.created_at, o.updated_at,
...
export async function POST(request) {
  ...
  const { hotel, proveedor, firma_encargado, nro_orden, items } = await request.json();
  if (!hotel || !proveedor || !items?.length) {
    return NextResponse.json({ error: 'Datos incompletos' }, { status: 400 });
  }
  const ordenRows = await query(
    `INSERT INTO ordenes (hotel, proveedor, firma_encargado, nro_orden, encargado_id, estado)
     VALUES ($1, $2, $3, $4, $5, 'PENDIENTE') RETURNING id`,
    [hotel, proveedor, firma_encargado, nro_orden || '', user.id]
  );
